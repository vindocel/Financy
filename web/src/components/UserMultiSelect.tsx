...
export default function UserMultiSelect({ familyId, value, onChange, onModeChange }: Props) {
  ...
  return (
    <div className="dropdown selector auto-s-1024" >
       <button className="selector-trigger auto-s-2002"
...
{(!allActiveSelected && value.length) ? (
  ...
  <span
    ref={labelRef}
    className="selector-value auto-s-1026"
...
<span aria-hidden>▾</span>
      ...
      {open && (
        <div className="dropdown-panel" role="listbox" aria-multiselectable={true} ref={listRef} onScroll={(e)=> setScrollTop((e.target as HTMLDivElement).scrollTop)} style={virtEnabled ? { maxHeight: viewportH } : undefined}>
          ...
          <div className="field auto-s-1027" ><label htmlFor="usr-q" className="sr-only">{t.search}</label><input id="usr-q" placeholder={t.search} value={q} onChange={(e)=>setQ(e.target.value)} autoFocus /></div>

          {/* Pills */}
          <div className="tags-grid auto-s-1028" >
            {/* Todos */}
            <button
              type="button"
